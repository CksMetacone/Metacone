...
public class WebCamText : MonoBehaviour
{
    ...
    public float captureIntervalSeconds = 0.01f;
    public int requestedWidth = 640;
    public int requestedHeight = 480;
    public FeatureType featureType = FeatureType.TEXT_DETECTION;
    public int maxResults = 5;

	public InputField inp_Eyes1;
	public InputField inp_Eyes2;
	public InputField inp_voice1;
	public InputField inp_voice2;


	string start_input ;
	string last_start_input;
	...
	void Start()
    {
		...
		{
			webcamTexture = new WebCamTexture(devices[1].name, requestedWidth, requestedHeight);
			Renderer r = GetComponent<Renderer>();
			if (r != null)
			{
				Material m = r.material;
    ...
    private IEnumerator Capture()
	{
		while (true)
		{
			...
			start_input = inp_voice1.text + inp_voice2.text;
			if ((start_input != "") && (last_start_input != start_input))
			{
				...
				if (jsonData != string.Empty)
				{
					...
					using (WWW www = new WWW(url, postData, headers))
					{
						...
						{
							yaztest = www.text.Replace("\n", "").Replace(" ", "");
							Debug.Log("yaztest");
							Debug.Log(yaztest);
							if (yaztest.IndexOf("description") != -1)
							{
								basla = yaztest.IndexOf("description") + 14;
								//Debug.Log("basla");
								//Debug.Log(basla);
								bitir = (yaztest.IndexOf("bounding") - 3) - basla;
								//Debug.Log("bitir");
								//Debug.Log(bitir);
								if(bitir>2)
									yazeyes1 = yaztest.Substring(basla, bitir - 2);
								else
									yazeyes1 = yaztest.Substring(basla, bitir);
								Debug.Log("yazeyes1");
								//Debug.Log(yazeyes1);
								yazeyes1 = yazeyes1.Replace(".", "");
								if((yazeyes1 == "ㅇ") || (yazeyes1 == "O") || (yazeyes1 == "о")  )
										yazeyes1 = "0" ;
								Debug.Log(yazeyes1);

								if(yazeyes1 != last_yazeyes1)
									last_start_input = start_input;
								last_yazeyes1 = yazeyes1;
								inp_Eyes1.text = yazeyes1.Substring(0, 1);
								if (yazeyes1.Length > 1)
									inp_Eyes2.text = yazeyes1.Substring(1, 1);
								else
									inp_Eyes2.text = "";
							}
							//Debug.Log("yaztest2");
							//Debug.Log(yaztest.IndexOf("description")+"\n");//+14 den sonrasını yaz
							//Debug.Log(yaztest.Substring(yaztest.IndexOf("description")+14), yaztest.IndexOf("bounding"));
